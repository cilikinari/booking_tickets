  ...
  class BookingProvider extends ChangeNotifier {
    ...
    // ==========================================
    // 🔥 UPDATE UNTUK MANAJEMEN LOADING & ERROR 🔥
    // ==========================================
    Future<void> processPayment({
      ...
      _isLoading = true;
      _errorMessage = '';
      notifyListeners(); // Nyalakan loading spinner di UI
      ...
      try {
        ...
        if (response.statusCode != 200 && response.statusCode != 201) {
          final error = jsonDecode(response.body);
          _errorMessage = error['message'] ?? 'Gagal memproses pembayaran';
          throw Exception(_errorMessage);
        }
      } catch (e) {
        _errorMessage = e.toString().replaceAll('Exception: ', '');
        rethrow; // Lempar error agar ditangkap blok try-catch UI Presentation kamu
      } finally {
        _isLoading = false;
        notifyListeners(); // Matikan loading spinner (Sukses / Gagal)
      }
    }
